...
async function getOptions() {
    await
        chrome.storage.sync.get({favoriteColor: 'cyan', hhUrl: 'someUrl'},
            (result) => {
                document.getElementById('add_article_form').setAttribute('action', result['hhUrl'] + '/plugin/add-article');
                return result;
            });
}
...
chrome.runtime.onMessage.addListener(
    (message, sender, sendResponse) => {
        ...
        console.log(`popup.js extension received message ${message.code}`);
        chrome.action.setBadgeText({"text": "??"});
        let color = "green";
